<template>
    <div class="p-2 flex flex-col prompt-container gap-2 md:p-4">
        <form class="flex gap-2" @submit="sendMessage" @submit.prevent>
            <InputText type="text" :placeholder="t('prompt.write')" class="w-full p-2 rounded"
            v-model="message" />
        ...
        <section class="flex flex-col gap-4 items-start md:flex-row md:gap-4 md:items-center mt-2">
        ...
        <Accordion value="" v-if="agent?.id && conv?.id">
            <AccordionPanel value="0">
                <AccordionHeader><i18n-t keypath="options.label"/></AccordionHeader>
                <AccordionContent>
                    <section class="flex flex-wrap gap-4">
                        <div v-for="option in SelectMode" :key="option" class="flex items-center gap-2">
                            ...
                            <label :for="option">
                                {{
                                    option === 'all' ? `${t('search.all')}` :
                                        option === 'select' ? `${t('search.sel')}` :
                                            `${t('search.dis')}`
                                }}
                            </label>
                        </div>
                        <section>
                            <MultiSelect :placeholder="t('filegroup.file.choice')" v-if="showFiles" :options="files" optionLabel="name"
