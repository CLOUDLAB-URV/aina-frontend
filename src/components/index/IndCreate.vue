<template>
  <form class="flex flex-col gap-4">
    <div class="flex flex-col gap-2">
      ...
      <InputText id="Id" :placeholder="t('input.name.placeholder')" v-model="model.name.value"
        :invalid="model.name.invalid" />
    </div>

    <div class="flex flex-col gap-2">
      <label for="vendorName">{{ t('input.index.label') }}</label>
      <Select v-model="model.indexType.value" :options="vendors" :placeholder="t('input.index.placeholder')"
...
import type { TableIndex } from "./IndkList.vue";
...
onMounted(async () => {
  ...
  vendors.value = response.map((vendor: string | null) => {
    return vendor ?? "";
  })
});
