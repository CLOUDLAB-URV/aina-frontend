                ...
                <TabPanels>
                    <TabPanel value="0">
                        ...
                        <DataTable :value="fileStore.files" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]"
                            stripedRows @row-click="madeClick" tableStyle="min-width: 50rem">
                    ...
                    <TabPanel value="1">
                        ...
                        <DataTable :value="groupStore.groups" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]"
                            ...
                            <Column field="files" header="Files" style="width: 70%"></Column>
                            <Column field="date_created" header="Date Created" style="width: 15%"></Column>
                        </DataTable>
                    </TabPanel>
                </TabPanels>
    ...
    async () => {
        if (agent.value.id) {
            ...
            let info = indexGroups.map((group: GroupInfo) => {
                let files = group.files.map((file) => notFiltered.find((f) => f.id === file)?.name).join(", ");
                return { ...group, files };
            });
